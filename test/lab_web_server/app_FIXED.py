...
# --- تابع مشترک پارس وضعیت NBCM ---
# قبلاً این منطق در ۳ جای مختلف فایل (با کلیدهای متفاوت "notactive" /
# "inactive") کاملاً کپی شده بود. یکی‌سازی آن ریسک واگرایی رفتار بین
# بخش‌های مختلف داشبورد (index.html در برابر plot_display.html) را از
# بین می‌برد. مقدار خروجی هماهنگ با چیزی است که index.html می‌خواند:
# indicator active <=> last.nbcm_statuses[key] === 'active'
def parse_nbcm(nbcm_str):
    status = {f'NBCM{i}': 'notactive' for i in range(1, 5)}
    if nbcm_str:
        for item in nbcm_str.split(','):
            key = item.strip()
            if key in status:
                status[key] = 'active'
    return status
...
DEFAULT_PORT = os.environ.get('ESP_SERIAL_PORT', 'COM9')
...
def get_master_data():
    ...
    for r in readings:
        ...
        if r.date and r.time:
            ...
            # باگ قبلی: '%Y-%m-%dTH:%M:%S' بدون % قبل از H بود، یعنی
            # حرف 'H' به‌جای ساعت به‌صورت متن خام چاپ می‌شد و Plotly در
            # حالت fallback زمان را اشتباه پارس می‌کرد.
            iso_timestamp = r.timestamp.strftime('%Y-%m-%dT%H:%M:%S')
...
# نکته مهندسی: قبلاً این پروژه دو تابع مختلف را روی همان مسیر
# '/api/sensor_data' رجیستر می‌کرد (این تابع، و یک get_sensor_data_api
# دیگر پایین‌تر در فایل). Flask/Werkzeug فقط اولین Rule ثبت‌شده برای یک
# مسیر تکراری را اجرا می‌کند، پس نسخه‌ی دوم هرگز فراخوانی نمی‌شد و صرفاً
# کد مرده و گمراه‌کننده برای توسعه‌ی بعدی بود. حذف شد و منطق مفیدش
# (parse_nbcm) داخل همین تابع یکپارچه شده است.
